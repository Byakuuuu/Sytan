...
(function() {
  ...
  (function() {    // * ROOMS ======================================================
    ...
    _.setRoomMaster = function(room) {
      this.room = room;
      this._isHost = true;
      return LOG.p("Vous êtes l'hôte du salon : " + this.room.name);
    };
    ...
    _.setRoomJoin = function(room) {
      this.room = room;
      this._isHost = false;
      return LOG.p("Vous avez rejoind le salon : " + this.room.name);
    };
    //TODO: установить флаг в NetMessage? что типо теперь send.to
  ...
  _.createLocal = function() {
    ...
    if (String.any(ConfigManager.netPlayerName)) {
      ...
      if ($gameTemp._tempPlayerNetworkName == null) {
        $gameTemp._tempPlayerNetworkName = "Joueur " + Math.randomInt(1000);
      }
...
(function() {  //╒═════════════════════════════════════════════════════════════════════════╛
  ...
  _.onDisconnect = function() {
    ...
    HUIManager.notifyError("Déconnecté du serveur");
    return ANNetwork.stop();
  };
...
(function() {  //╒═════════════════════════════════════════════════════════════════════════╛
  ...
  _._showNameInput = function() {
    HUIManager.showInput("Entrez votre message...");
...
Scene_NetworkGameMenu = class Scene_NetworkGameMenu extends Scene_MenuBase {
  ...
  refreshWelcomeText() {
    var e, ref;
    try {
      return (ref = this._welcomeLine) != null ? ref.drawTextFull("Bienvenue, " + ANGameManager.myPlayerData().name) : void 0;
    } catch (error) {
      e = error;
      return ANET.w(e);
    }
  ...
  refreshPlayersCountText(count = 0) {
    ...
    try {
      ...
      return this._playerCountText.drawTextFull("Joueurs connectés : " + count);
    } catch (error) {
      e = error;
      return ANET.w(e);
    }
...
(function() {
  ...
  _._onConnectionRefused = function() {
    HUIManager.hideLoader();
    HUIManager.notifyError("Le serveur ne répond pas.");
    return this.popScene();
  };
  _._onConnectionGood = function() {
    ...
    HUIManager.notifySucess("Connecté au serveur");
  ...
  _._initSceneComponents = function() {
    ...
    HUIManager.showInput("Nom du salon");
...
(function() {
  ...
  _.requestRoomsListFromServer = function() {
    ANNetwork.get(NMS.Lobby("getRoomsList"), (result) => {
      ...
      // * Timeout
      console.log("Le serveur ne renvoie pas la liste de salon à temps.");
  ...
  _._onCantJointRandomRoom = function() {
    ...
    HUIManager.notifyError("Pas de salon disponnible à rejoindre");
  };
...
(function() {  //╒═════════════════════════════════════════════════════════════════════════╛
  ...
  _._refreshRoomTitle = function() {
    ...
    if (ANNetwork.isMasterClient()) {
      roomHostName = "\\C[1]" + ANGameManager.myPlayerData().name + " (vous)";
    } else {
      if (this.room == null) {
        roomHostName = "Recherche...";
      } else {
        roomHostName = (ref = ANGameManager.getPlayerDataById(this.room.masterId)) != null ? ref.name : void 0;
      }
    }
    return this._helpWindow.setText("Salon: %1, Hôte: %2".format(ANNetwork.room.name, roomHostName));
  };
...
(function() {  // ■ END Scene_NetworkRoomTypeSelect.coffee
  ...
  _._createNewRoom = function(uniqueSaveId) {
    ...
    ANNetwork.get(NMS.Lobby("createRoom", newRoomData), (result) => {
      ...
      console.log("Imposible de créer le salon, le serveur ne répond pas.");
      return this._commandsWindow.activate();
    });
  };
  //?EVENT
  _._onRoomCreated = function(roomData) {
    if (roomData != null) {
      ...
      HUIManager.notifyError("Impossible de créer un salon avec le nom: " + this._lastRoomName);
      this._commandsWindow.activate();
    }
  };
...
(function() {  //╒═════════════════════════════════════════════════════════════════════════╛
  ...
  _._showNameInput = function() {
    HUIManager.showInput("Entrez votre pseudonyme...");
          ...
          return ANNetwork.get(NMS.Lobby("createRoom", {
            ...
            if (roomData != null) {
              ...
              return ANNetwork.get(NMS.Lobby("joinToRoom", "dev"), function(roomData) {
                ...
                return console.log("Impossible de rejoindre le salon, le serveur de répond pas.");
              });
            }
          }, function() {
            return console.log("Impossible de créer le salon, le serveur ne répond pas.");
          });
        } else {
          return HUIManager.notifyError("Le serveur ne répond pas.");
        }
      });
    };
...
Window_NetworkGameMenu = class Window_NetworkGameMenu extends Window_Command {
  ...
  makeCommandList() {
    this.addCommand("Créer un salon", "createRoom");
    this.addCommand("Rejoindre un salon", "joinRoom");
    this.addCommand("Rejoindre un salon aléatoire", "joinRandRoom");
    this.addCommand("Paramètres", "settings");
  }
...
Window_NetworkRoomCommands = class Window_NetworkRoomCommands extends Window_HorzCommand {
  ...
  makeCommandList() {
    ...
    if (ANNetwork.isMasterClient()) {
      this.addCommand('Commencer', 'start', this._isStartEnabled()); //TODO: Третий аргумент : enabled
    } else {
      //TODO: Надо проверять все ли готовы, только тогда кнопка активна
      //TODO: Ещё можно проверять больше 1 игрока или нет
      this.addCommand('Prêt', 'ready', false);
    }
    //TODO: Пока отключим, нет функционала
    if (this.isCanSelectActors()) {
      this.addCommand("Personnage", 'character', this._isCharSelectEnabled());
    }
    leaveCommandName = ANNetwork.isMasterClient() ? "Fermer" : "Leave";
...
(function() {  //╒═════════════════════════════════════════════════════════════════════════╛
  ...
  _._createNoRoomsMessage = function() {
    ...
    this._noRoomsTextSpr.drawText("Il n'y a pas de salons sur le serveur");
...
Window_NetworkRoomTypeMenu = class Window_NetworkRoomTypeMenu extends Window_Command {
  ...
  makeCommandList() {
    this.addCommand("Nouvelle partie", "newGame");
    this.addCommand("Charger partie", "continue", this.isHaveSavedGames());
  }
...
(function() {
  ...
  _.drawTitle = function(savefileId, x, y) {
    if (DataManager.nIsNetworkSaveFile(savefileId)) {
      return this.drawText(TextManager.file + " " + savefileId + " [Partie multijoueur]", x, y, 240);
    } else {
...
(function() {
  ...
  _.makeCommandList = function() {
    ...
    this.addCommand('Multijoueur', "network");
